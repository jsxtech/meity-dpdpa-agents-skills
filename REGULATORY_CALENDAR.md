# DPDP Regulatory Calendar

> **Status note:** Dates are indicative based on publicly available information as of April 2026. Verify against official MeITY/Gazette notifications.

---

## Key Dates & Events

| Date / Period | Event | Status | Impact | Action Required |
|---|---|---|---|---|
| 11 Aug 2023 | DPDP Act enacted (No. 22 of 2023) | ✅ Completed | Full personal data protection framework in force | Baseline compliance programme required |
| Jan 2025 | Draft DPDP Rules published for public consultation | ✅ Completed | Detailed implementation requirements visible | Review draft Rules; submit consultation response; gap analysis against draft |
| 13 Nov 2025 | Final DPDP Rules, 2025 notified and gazetted (14 Nov 2025) | ✅ Completed | Binding timelines, formats, thresholds now statutory | Update all agents/skills; revise compliance programme; brief leadership |
| Nov 2025 | Data Protection Board of India (DPBI) constituted | ✅ Completed | Enforcement, complaints, and adjudication active | Complaint response procedures in place; DPBI liaison appointed |
| TBD | Significant Data Fiduciary (SDF) designation notifications | ⏳ Pending | Additional obligations for designated entities | SDF readiness assessment; DPO appointment; auditor engagement |
| TBD | Consent Manager registration framework operational | ⏳ Pending | Registered Consent Managers become operational | Evaluate Consent Manager integration; update consent flows |
| TBD | Permissible countries list for cross-border transfers | ⏳ Pending | Transfers restricted to notified countries only | Audit all cross-border data flows; update Transfer Register |
| Ongoing | RBI data localisation compliance | 🟢 Active | Payment system data must be stored in India | Maintain data residency controls; periodic audit |
| Ongoing | SEBI cyber security circular compliance | 🟢 Active | Cyber resilience requirements for market intermediaries | Align security controls with SEBI framework |
| Ongoing | IRDAI information security guidelines | 🟢 Active | Data protection requirements for insurers | Map IRDAI requirements to DPDP controls |
| Ongoing | CERT-In 6-hour incident reporting | 🟢 Active | Cyber incidents must be reported within 6 hours | Maintain incident response playbook; test reporting SLA |

---

## How to Use This Calendar

1. **Review monthly** — check for gazette notifications and regulatory circulars
2. **Assign owners** — each pending item should have a named responsible person
3. **Set alerts** — subscribe to MeITY and DPBI notification feeds
4. **Trigger updates** — when a pending item moves to completed, update all affected agents and skills using the Regulatory Monitoring Agent

---

## Monitoring Sources

| Source | URL / Channel | What to Watch |
|---|---|---|
| MeITY Gazette notifications | egazette.gov.in, meity.gov.in | DPDP Rules final notification, SDF designations, country list |
| DPBI website | *(to be established)* | Board orders, complaint procedures, registration forms |
| RBI circulars | rbi.org.in | Data localisation updates, payment data storage directives |
| SEBI circulars | sebi.gov.in | Cyber security framework updates, data handling requirements |
| IRDAI circulars | irdai.gov.in | Information security and data protection guidelines |
| CERT-In advisories | cert-in.org.in | Incident reporting requirements, vulnerability alerts |
| TDSAT orders and procedures | tdsat.gov.in | DPDP appeal procedures, orders, timelines |
